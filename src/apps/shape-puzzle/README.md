...
3. **Toolkit-Level Action Integration**: Shape-puzzle commands are recognized by the shared voice intent resolver and translated by the fusion core into canonical actions such as `create_object`, `set_interaction_mode`, `insert_object`, and `update_pointer`. The app consumes these actions through `handle_action()` instead of monkey-patching toolkit vocabulary or parsing raw transcripts itself.
4. **Physicality Stabilization Mapping**: The fusion core emits pointer, palm-delta, and pinch-delta action metadata. The puzzle uses this shared action data for drag, rotation, and resizing while keeping scoring and rendering inside the app.
